...
import re
import requests
from dy_utils.dy_util import download_media, check_and_create_path, norm_str, get_headers, handle_video_info, \
    check_info, save_video_detail
...
class OneVideo:
    ...
    def get_one_video_info(self, url):
        try:
            response = requests.get(url, headers=self.headers,
                                    cookies={"s_v_web_id": self.info['cookies']["s_v_web_id"]})
            html_text = response.text
            video_info = re.findall('<script id="RENDER_DATA" type="application/json">(.*?)</script>', html_text, re.S)[
                0]
        except:
    ...
    def save_one_video_info(self, url, need_cover=False):
        ...
        save_video_detail(path, video)  # 视频信息，可选择删除
...
if __name__ == '__main__':
    one_video = OneVideo()
    url_list = [
        "https://www.douyin.com/user/MS4wLjABAAAAzzmS2TgIEvxGftMpWD13Ty8k5HmsjlGsLJ1yBUEm2Ew?modal_id=7263020179653791030",
        "https://www.douyin.com/user/MS4wLjABAAAAzzmS2TgIEvxGftMpWD13Ty8k5HmsjlGsLJ1yBUEm2Ew?modal_id=7324236237143870755"
    ]
